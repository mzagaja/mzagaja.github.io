...
module Jekyll
  module WordPress
    ...
    # have valid dates.
    QUERY = "select post_title, post_name, post_date, post_content, post_excerpt, ID, guid from wp_sm0kxv_posts where post_status = 'publish' and post_type = 'post'"
	# Fetch all tags for a given POST ID
	TAGS_QUERY = "select tm.term_id,tm.name from wp_sm0kxv_term_relationships tr 
				  inner join wp_sm0kxv_term_taxonomy tt on tr.term_taxonomy_id = tt.term_taxonomy_id 
				  inner join wp_sm0kxv_terms tm on tm.term_id=tt.term_id  
				  where tr.object_id=%d and tt.taxonomy = 'post_tag'";
 
    def self.process(dbname = 'zagaja_com_2', user='zagajacom2', pass='Zjcn9Kpf', host = 'drupal.zagaja.com', domain = '')
	...
	def self.transformUrls(domain,content)
		...
		return content.gsub(baseurl,"zagaja.com/images/")
	end
